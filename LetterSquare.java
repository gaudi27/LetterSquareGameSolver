...
public class LetterSquare {
    ...
    private boolean isValid(String letter, int wordNum, int charNum) {        
        ...
        else if (charNum == 0 && lastLetter(words[wordNum - 1]).equals(letter)){
            return true;
        }
        else if (onSameSide(letter, words[wordNum].substring(charNum -1, charNum))){
            return false;
        }
        else if (alreadyUsed(words[wordNum] + letter)){
            return false;
        }
        else if (dictionary.hasString(words[wordNum] + letter) == true){
            return true;
        }
        else {
            return false;
    ...
    private boolean solveRB(int wordNum, int charNum, int maxWords) {
        /* Replace this line with your implementation of the method. */
        if (this.allLettersUsed() && dictionary.hasFullWord(words[wordNum])){
        ...
        for (int i = 0; i < letters.length; ++i){
            if (this.isValid(letters[i], wordNum, charNum)){
                ...
                else if (charNum >= 3 && dictionary.hasFullWord(words[wordNum])){
